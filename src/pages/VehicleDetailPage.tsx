    ...
    <div className="border-b border-gray-100 last:border-b-0">
      ...
      <div
        className={`overflow-hidden transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          open ? 'max-h-[2000px] opacity-100 pb-6' : 'max-h-0 opacity-0'
        }`}
      >
        {children}
      </div>
...
function SpecItem({
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-3.5 rounded-xl bg-gray-50/70 border border-gray-100/80">
      <div className="text-smartlease-blue/60 flex-shrink-0">{icon}</div>
...
export function VehicleDetailPage() {
  ...
  const images = useMemo<string[]>(() => {
    if (!vehicle) return [];
    const imgs: string[] = [];
    const count = vehicle.images?.length || 0;
    if (vehicle.external_id && count > 0) {
      for (let i = 1; i <= count; i++) {
        imgs.push(proxyLargeImage(vehicle.external_id, i));
      }
    } else if (vehicle.external_id) {
      imgs.push(proxyThumb(vehicle.external_id));
    } else if (vehicle.small_picture) {
      imgs.push(vehicle.small_picture);
    }
    return imgs;
  }, [vehicle]);

  const thumbImages = useMemo<string[]>(() => {
    if (!vehicle) return [];
    const thumbs: string[] = [];
    const count = vehicle.images?.length || 0;
    if (vehicle.external_id && count > 0) {
      for (let i = 1; i <= count; i++) {
        thumbs.push(proxyLargeImage(vehicle.external_id, i));
      }
    }
    return thumbs;
  }, [vehicle]);
  ...
  useEffect(() => {
    if (!vehicle?.external_id || images.length <= 1) return;
    const toPreload = [1, 2, 3, -1];
    toPreload.forEach((offset) => {
      const idx = (currentImageIndex + offset + images.length) % images.length;
      if (images[idx]) preloadImage(images[idx]);
    });
  }, [currentImageIndex, vehicle, images, preloadImage]);
  ...
  const handleOfferteNavigate = () => {
    if (!vehicle) return;
    navigate('/offerte', { state: vehicleState() });
  };

  const handleBelMijNavigate = () => {
    if (!vehicle) return;
    navigate('/bel-mij', { state: vehicleState() });
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-8">
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                ...
                <div className="flex flex-col items-start md:items-end">
                  {vehicle.maandprijs > 0 ? (
                    ...
                    <span className="text-3xl md:text-4xl font-bold text-smartlease-blue">{formatPrice(vehicle.verkoopprijs)}</span>
                  )}
                </div>
              </div>
            ...
            <div className="animate-fade-up opacity-0 delay-4 bg-white rounded-2xl shadow-sm px-5 md:px-7">
              ...
              {/* ── Verkoper — alleen zichtbaar voor admin ── */}
              {isAdmin && (
